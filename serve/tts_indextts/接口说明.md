...
| `prompt_audio` | string | 是 | - | 说话人参考音频文件路径 |
| `emo_mode` | string | 是 | `"none"` | 情感控制模式：`none`/`audio`/`vector`/`text` |
| `interval_silence` | int | 否 | `200` | 句子间静音时长（毫秒） |
| `max_tokens` | int | 否 | `120` | 每段最大 Token 数 |
| `use_random` | bool | 否 | `false` | 是否启用随机性 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `emo_audio` | string | 是 | - | 情感参考音频路径 |
| `emo_alpha` | float | 否 | `1.0` | 情感权重（0.0-1.0） |

**情感向量模式 (`vector`)**

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `emo_vector` | array | 是 | - | 8 维情感向量 [高兴, 愤怒, 悲伤, 恐惧, 反感, 低落, 惊讶, 自然] |

**情感文本引导模式 (`text`)**

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `emo_text` | string | 是 | - | 情感引导文本 |
| `emo_alpha` | float | 否 | `1.0` | 情感权重（0.0-1.0） |
