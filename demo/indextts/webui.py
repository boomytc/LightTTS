...
parser.add_argument("--model_dir", type=str, default="models/IndexTTS-2", help="Model checkpoints directory")
...
with gr.Blocks(title="IndexTTS Demo") as demo:
    ...
    gr.HTML('''
    <h2><center>IndexTTS2：情感表达丰富且时长可控的自回归零样本文本到语音转换的突破</h2>
    ''')

    with gr.Tab("音频生成"):
        with gr.Row():
            prompt_audio = gr.Audio(label="音色参考音频",key="prompt_audio",
                                    sources=["upload","microphone"],type="filepath")
            with gr.Column():
                input_text_single = gr.TextArea(label="文本",key="input_text_single", placeholder="请输入目标文本", info=f"当前模型版本{tts.model_version or '1.0'}")
        ...
        with gr.Accordion("高级生成参数设置", open=False, visible=True) as advanced_settings_group:
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown(f"**GPT2 采样设置** _参数会影响音频多样性和生成速度详见 [Generation strategies](https://huggingface.co/docs/transformers/main/en/generation_strategies)._")
                    with gr.Row():
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        ...
                        max_text_tokens_per_segment = gr.Slider(
                            label="分句最大Token数", value=initial_value, minimum=20, maximum=tts.cfg.gpt.max_text_tokens, step=2, key="max_text_tokens_per_segment",
